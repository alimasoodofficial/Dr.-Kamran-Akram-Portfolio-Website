  ...
  return (
    <section id="about" className="bg-white dark:bg-slate-900 rounded-3xl p-8 md:p-10 shadow-sm border border-slate-100 dark:border-slate-800 relative overflow-hidden mb-20">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-3 font-heading">
            <span className="w-10 h-1 bg-green-600 rounded-full"></span> About Me
        </h2>
        
        <div className="grid md:grid-cols-1 gap-6 text-slate-600 dark:text-slate-300 leading-relaxed text-lg font-body text-justify">
            <p>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
            {tags.map((tag, index) => (
                <span key={index} className="px-4 py-2 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-lg text-sm font-semibold border border-slate-200 dark:border-slate-700 hover:bg-green-600 hover:text-white transition-colors cursor-pointer hover:scale-110">
                    {tag}
                </span>
            ))}
        </div>
