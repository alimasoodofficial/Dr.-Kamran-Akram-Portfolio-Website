...
const ArticleBook: React.FC<ArticleBookProps> = ({
  ...
  const coverRotation = isOpen ? "[transform:rotateY(-150deg)]" : "";

  const imageScale = isOpen ? "grayscale-0 scale-110" : "grayscale";
  ...
  return (
    ...
    >
      {/* Book Wrapper */}
      <div
        className={`relative w-full h-full transition-transform duration-1000 ease-in-out [transform-style:preserve-3d] ${containerRotation}`}
      >
        {/* --- THE MOVING COVER --- */}
        <div
          className={`absolute inset-0 z-20 transition-transform duration-1000 ease-in-out origin-left [transform-style:preserve-3d] ${coverRotation}`}
        >
          {/* Front of Cover */}
          <div className="absolute inset-0 flex flex-col justify-between p-4 md:p-8 text-white border border-slate-600 rounded-r-md bg-gradient-to-br from-slate-700 to-slate-900 [backface-visibility:hidden] shadow-inner">
            ...
            <div className="mt-4 backdrop-blur-sm bg-white/10 border border-white/20 p-2 md:p-4 rounded-lg">
              ...
              {author && (
                <p className="text-[8px] md:text-xs text-gray-300 mt-2">
                  By {author}
                </p>
              )}
            </div>
        ...
        <div className="absolute top-1 left-1 w-[calc(100%-8px)] h-[calc(100%-8px)] bg-white rounded-r-md z-10 overflow-hidden shadow-sm border-r border-slate-200">
          <Image
            ...
            alt={title}
            fill
            className={`px-7 object-contain object-center transition-all duration-700 ${imageScale}`}
            placeholder="blur"
            blurDataURL="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mN8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg=="
          />
          {/* CTA Overlay - Only clickable when open */}
          <div
            className={`absolute inset-0 bg-black/10 transition-colors flex items-center justify-center ${isOpen ? "pointer-events-auto" : "pointer-events-none"}`}
          >
            <button
              onClick={handleReadClick}
              className={`bg-[#E67E22] px-5 py-2 rounded-full text-xs font-bold text-white shadow-xl transform transition-all duration-500 delay-100 hover:bg-[#D35400] hover:text-white ${buttonOpacity}`}
