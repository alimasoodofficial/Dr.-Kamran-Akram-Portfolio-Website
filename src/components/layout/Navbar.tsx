...
import { ebookMenuItems } from "@/data/ebookMenu";
        ...
        <div className="hidden lg:flex gap-4 font-body font-medium ml-auto mr-8 items-center relative text-black dark:text-white">
          ...
          <div className="group relative px-3 py-2">
            ...
            <div
              ...
              absolute top-full left-1/2 -translate-x-1/2 mt-4 
              bg-white dark:bg-[#1a1b26] border border-gray-100 dark:border-gray-700
              rounded-2xl shadow-2xl backdrop-blur-lg p-6 z-50 min-w-[550px]"
            >
              <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-6 border-b border-gray-100 dark:border-gray-700 pb-4 font-heading">
                eBooks & Publications
              </h3>

              <div className="grid grid-cols-1 gap-4">
                {ebookMenuItems.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="flex items-start gap-4 p-4 rounded-xl hover:bg-gray-50 dark:hover:bg-white/5 transition-all group/item"
                  >
                    <div className={`w-12 h-12 ${item.bgColor || "bg-gradient-to-br from-emerald-500 to-green-600"} rounded-lg flex items-center justify-center shadow-lg group-hover/item:scale-110 transition-transform flex-shrink-0`}>
                      <i className={`${item.icon} text-white text-xl`} aria-hidden="true" />
                    </div>
                    <div className="flex-1">
                      <p className="font-bold text-gray-900 dark:text-white mb-1">
                        {item.title}
                      </p>
                      <p className="text-sm text-gray-650 dark:text-gray-405 leading-tight">
                        {item.description}
                      </p>
                    </div>
                  </Link>
          ...
          {/* Other Links */}
          {/* Filter out 'Free Resources', 'About Me', 'Resume', 'Projects', 'Gallery', 'eBooks' from navigation */}
          {navLinks
            .filter(
              (link) =>
              ...
              <div className="grid grid-cols-1 gap-4">
                {ebookMenuItems.map((item) => (
                  <Link
                    ...
                    className="flex items-start gap-4 p-4 rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 transition-all border border-transparent hover:border-gray-200 dark:hover:border-gray-700"
                  >
                    <div className={`w-12 h-12 ${item.bgColor || "bg-gradient-to-br from-emerald-500 to-green-600"} rounded-lg flex items-center justify-center shadow-lg flex-shrink-0`}>
                      <i className={`${item.icon} text-white text-xl`} aria-hidden="true" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-gray-900 dark:text-white text-base mb-1">
