...
export default function StatusClient() {
  ...
  useEffect(() => {
    ...
    if (successParam) {
      sessionStorage.setItem("booking_success", successParam);
    } else {
      successParam = sessionStorage.getItem("booking_success");
    }
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center px-4">
        <div className="p-10 max-w-md w-full bg-slate-900/40 backdrop-blur-xl border border-emerald-500/20 rounded-[2.5rem] text-center shadow-2xl space-y-6">
          <Loader2 className="w-16 h-16 text-primary animate-spin mx-auto" />
          <div className="space-y-3">
            <h3 className="text-2xl font-black tracking-tight text-white">Confirming Booking</h3>
  ...
  if (error || !booking) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center px-4 py-16">
        <div className="p-10 max-w-xl w-full bg-slate-900/40 backdrop-blur-xl border border-red-500/20 rounded-[2.5rem] text-center shadow-2xl space-y-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4">
            <Link
              href="/consulting"
              className="flex-1 h-14 bg-primary hover:bg-[#064e3b] text-white text-xs font-black uppercase tracking-widest rounded-2xl flex items-center justify-center transition-all duration-300 shadow-lg shadow-primary/20"
            >
              Try Booking Again
            </Link>
            <Link
              href="/"
              className="flex-1 h-14 bg-slate-800 hover:bg-slate-700 text-white text-xs font-black uppercase tracking-widest rounded-2xl flex items-center justify-center border border-slate-700 transition-all duration-300"
            >
              Go to Home Page
            </Link>
          </div>
  ...
  return (
    <div className="min-h-[85vh] flex flex-col items-center justify-center px-4 py-16">
      <div className="p-8 md:p-12 max-w-2xl w-full bg-slate-900/30 backdrop-blur-xl border border-emerald-500/15 rounded-[3rem] shadow-2xl space-y-8 relative overflow-hidden">
        {/* Glow decoration */}
        ...
        <div className="text-center space-y-3">
          <h2 className="text-3xl md:text-4xl font-black tracking-tight text-white">Booking Confirmed!</h2>
        ...
        {/* Details Card */}
        <div className="p-6 md:p-8 bg-slate-900/60 dark:bg-slate-950/40 rounded-[2rem] border border-slate-800 space-y-6">
          <h3 className="text-xs font-black uppercase tracking-widest text-primary border-b border-slate-800 pb-3 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="font-bold text-white text-sm">{booking.fullName}</p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="font-bold text-white text-sm">{displayDate}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="font-bold text-white text-sm">{booking.timeSlot} ({booking.duration} mins)</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="font-bold text-white text-sm">{booking.platform}</p>
              </div>
            </div>
          </div>
        ...
        {/* Email note */}
        <div className="p-5 bg-slate-900/40 rounded-2xl border border-slate-800 text-center text-xs text-slate-400 font-medium leading-relaxed">
          💡 A calendar invitation and confirmation email with these details have been sent to <span className="text-white font-bold">{booking.email}</span>.
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-2">
          ...
          <Link
            href="/"
            className="flex-1 h-14 bg-slate-850 hover:bg-slate-800 text-white text-xs font-black uppercase tracking-widest rounded-2xl flex items-center justify-center gap-2 border border-slate-800 transition-all duration-300"
          >
