...
import { Plus, Edit, Trash2, Search, Book, Download, ExternalLink } from "lucide-react";
...
export default function AdminEbooks() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
        const { error } = await supabaseClient.from("ebooks").delete().eq("id", id);
        if (error) throw error;
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage digital downloads and resources.</p>
        </div>
                ...
                {filtered.map((ebook) => (
                    ...
                    >
                         <div className="aspect-[2/3] bg-slate-100 relative overflow-hidden">
                            {ebook.cover_url ? (
                                <img src={ebook.cover_url} alt={ebook.title} className="w-full h-full object-cover transition-transform group-hover:scale-105" />
                            ) : (
                         ...
                         <div className="p-4">
                             <h3 className="font-bold text-slate-800 line-clamp-1" title={ebook.title}>{ebook.title}</h3>
