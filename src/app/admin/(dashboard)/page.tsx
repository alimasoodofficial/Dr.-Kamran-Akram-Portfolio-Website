...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function fetchStats() {
      ...
      const [galleryCount, articlesCount, ebooksCount, subscribersCount] = await Promise.all([
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-500 mt-2">Welcome back, Admin. Here is what is happening.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {cards.map((card) => (
          ...
          >
            <div className={`absolute top-0 right-0 p-3 opacity-10 group-hover:opacity-20 transition-opacity ${card.color} rounded-bl-3xl`}>
                <card.icon className="w-16 h-16 text-white" />
            </div>
            
            <div className="relative z-10 flex flex-col justify-between h-full">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${card.color} bg-opacity-10 mb-4`}>
